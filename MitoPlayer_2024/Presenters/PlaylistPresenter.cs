    ...
    public class PlaylistPresenter
    {
        ...
        public PlaylistPresenter(IPlaylistView view, ITrackDao trackDao, ITagDao tagValueDao, ISettingDao settingDao)
        {
           ...
           // this.playlistView.SetQuickListEvent += SetQuickListEvent;
            ...
            this.playlistView.ChangeFilterModeEnabled += ChangeFilterModeEnabled;
            this.playlistView.ChangeOnlyPlayingRowModeEnabled += ChangeOnlyPlayingRowModeEnabled;
            this.playlistView.ChangeFilterText += ChangeFilterText;
        ...
        private void InitializeTaglist()
        {
            ...
            if (tagList != null && tagList.Count > 0)
            {
                ...
                tagValueListList = new List<List<TagValue>>();
                foreach (Tag tag in this.tagList)
                {
                    ...
                    if(tagValueList != null && tagValueList.Count > 0){
                        tagValueListList.Add(tagValueList);
                    }
                }
            }
            this.playlistView.SetTagsAndTagValues(this.tagList, this.tagValueListList);
        }

        private BindingSource playlistListBindingSource { get; set; }
        private DataTable playlistListTable { get; set; }
        private bool[] playlistColumnVisibilityArray { get; set; }
        ...
        private void InitializePlaylistDataTable()
        {
            ...
            ///a szinezés miatt előre le kell kérni és átadni/beállítani
            int currentPlaylistId = this.settingDao.GetIntegerSetting(Settings.CurrentPlaylistId.ToString());
            int playlistId = -1;

            for (int i = 0; i < this.playlistListTable.Rows.Count; i++)
            {
                playlistId = Convert.ToInt32(playlistListTable.Rows[i]["Id"]);
            ...
            this.playlistView.InitializePlaylistListBindingSource(this.playlistListBindingSource, this.playlistColumnVisibilityArray, this.currentPlaylistId);
        }
        
        private void ReloadPlaylistDatagGridView(DataTable playlistListTable)
        {
            int currentPlaylistId = this.settingDao.GetIntegerSetting(Settings.CurrentPlaylistId.ToString());
            int playlistId = -1;
            ...
            this.playlistView.ReloadPlaylistListBindingSource( this.playlistColumnVisibilityArray, this.currentPlaylistId);
        }
        ...
        private int[] TrackColumnSortingIdArray { get; set; }
        ...
        private void InitializeTracklistDataTable()
        {
            ...
            this.playlistView.InitializeTrackListBindingSource(this.trackListBindingSource, this.TrackColumnVisibilityArray, this.TrackColumnSortingIdArray);
        }
        ...
        private void ReloadTrackListDataGridView(DataTable trackListTable)
        {
            ...
            this.trackListBindingSource.DataSource = trackListTable;
            this.playlistView.ReloadTrackListBindingSource(this.trackListBindingSource, this.TrackColumnVisibilityArray, this.TrackColumnSortingIdArray, currentTrackIdInPlaylist);
        }
        ...
        public void ReloadData(MediaPlayerComponent mediaPlayer)
        {
            try
            {
                if(this.mediaPLayerComponent == null)
        ...
        private void InitializeTrackList()
        {
            ...
            List<Track> trackList = this.trackDao.GetTracklistByPlaylistId(actualPlaylist.Id, this.tagList);
       ...
       /* private void ChangeColoringByTagValues(String tagName)
        ...
        public void Shuffle()
        {
            if (this.trackListTable != null && this.trackListTable.Rows != null && this.trackListTable.Rows.Count > 0)
            {
                ...
                this.SaveTrackList(this.trackListTable, this.currentPlaylistId);
                this.ReloadTrackListDataGridView(trackListTable);
            }
        }
        ...
        private void CreatePlaylist(object sender, EventArgs e)
        {
            ...
            if (playlistEditorView.ShowDialog((PlaylistView)this.playlistView) == DialogResult.OK)
            {
                ...
                this.ReloadPlaylistDatagGridView(playlistListTable);
            }
        }
      ...
      /*  private void SetQuickListEvent(object sender, ListEventArgs e)
        ...
        private void ChangeFilterModeEnabled(object sender, ListEventArgs e)
        {
            this.isFilterModeEnabled = e.BooleanField1;
            this.tagValueFilterDic = new Dictionary<String, String>();
           // this.settingDao.SetBooleanSetting(Settings.IsFilterModeEnabled.ToString(), this.isFilterModeEnabled);
            ...
            if (!this.isFilterModeEnabled)
            {
                ...
                this.playlistView.FilterTracklist(this.trackListBindingSource, this.TrackColumnVisibilityArray, this.TrackColumnSortingIdArray, this.mediaPLayerComponent.CurrentTrackIdInPlaylist);
                this.mediaPLayerComponent.SetWorkingTable(this.trackListTable);
            }
        }
            ...
            else
            {
                if (this.tagList != null && this.tagList.Count > 0)
                {
                    ...
                    if (currentTag != null)
                    {
                        if (tagValueFilterDic.ContainsKey(currentTag.Name))
                        {
                            tagValueFilterDic.Remove(currentTag.Name);
                        }
                    }
                }
                ((PlaylistView)this.playlistView).SetTagValueFilter(this.tagValueFilterDic);
        ...
        {
            tagValueFilterDic = new Dictionary<String, String>();
            ((PlaylistView)this.playlistView).SetTagValueFilter(this.tagValueFilterDic);
        }
        private void ChangeFilterText(object sender, ListEventArgs e)
        {
            this.InitializeTrackList();
            ...
            if (!String.IsNullOrEmpty(filterText) || this.isFilterModeEnabled)
            {
                ...
                if (this.isFilterModeEnabled)
                {
                    if (this.tagValueFilterDic != null && this.tagValueFilterDic.Count > 0)
                    {
                        foreach (KeyValuePair<String, String> filter in this.tagValueFilterDic)
                        {
                            if (String.IsNullOrEmpty(filterQuery2))
                            {
                                filterQuery2 = filter.Key + " LIKE '%" + filter.Value + "%' ";
                            }
                            else
                            {
                                filterQuery2 = filterQuery2 + " AND " + filter.Key + " LIKE '%" + filter.Value + "%' ";
                            }
                        }
